...
- `options`: `<Object>`
  ...
  - `[sessionKey]`: <string> Optional argument, explicitly define the key in your session which is used for storing and retrieving the code_verifier. When it is not defined the sessionKey will be `oidc:` + issuer's hostname.
